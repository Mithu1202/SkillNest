...
const ShowcasesPage = () => {
  ...
  const fetchShowcases = async (uid) => {
    try {
      ...
      const initialVisibility = res.data.reduce(
        (acc, showcase) => ({
          ...acc,
          [showcase.id]: false,
        }),
  ...
  const handleEdit = (showcase) => {
    ...
    setMediaType(
      showcase.filePath ? "upload" : showcase.projectUrl ? "link" : "upload"
    );
  ...
  const handleDelete = async (showcaseId) => {
    ...
    try {
      ...
      toast.error(
        `Failed to delete showcase: ${err.response?.data?.error || err.message}`
      );
    }
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      toast.error(
        err.response?.data?.error || `Operation failed: ${err.message}`
      );
    } finally {
      setIsLoading(false);
    }
      ...
      <div className="space-y-4 mb-8">
        {showcases.map((showcase) => (
          ...
          >
            ...
            {showcase.filePath?.match(/\.(jpeg|jpg|png)$/i) &&
            imageUrls[showcase.id] ? (
            ...
            <div className="p-6 space-y-4">
              ...
              <div>
                ...
                <div className="flex space-x-2">
                  ...
                  <button
                    ...
                    disabled={
                      isLoading || skills.length >= 5 || !newSkill.trim()
                    }
                  >
                    Add
                  </button>
              ...
              <div>
                ...
                >
                  ...
                  <option value="hidden">
                    Hidden (Only accessible via link)
                  </option>
                </select>
              </div>
